...
@Controller
public class GPayReceiver {

	protected static final Logger logger = LoggerFactory
			.getLogger(GPayReceiver.class);
	...
	@RequestMapping(value = "/api/integrate/gpay/receiver", method = {
		...
		try {
			...
			if (b) {
				...
				if(_gpayRet.getOut_trade_no().length()>16){
					...
					PaymentInformation pi=paymentService.getOrderByOrderNo(paymentInformation.getOrderNo());
					...
					paymentService.updateSuccessIpayUserPaymentLogByOrderNo(paymentService,paymentInformation);
				}else{				
					paymentInformation.setId(Long.parseLong(_gpayRet.getOut_trade_no()));
					PaymentInformation pi=paymentService.getOrderByOrderId(paymentInformation.getId());
					if((Constants.PRICE_TYPE_MONTHLY+"").equals(pi.getMyPriceType())){
						Calendar cal=Calendar.getInstance();
						int amount=1;
						if(pi.getMonthlyCycle()!=null&&pi.getMonthlyCycle()>=1){
					...
					paymentService.updateSuccessIpayUserPaymentLog(paymentService,paymentInformation);
				}
