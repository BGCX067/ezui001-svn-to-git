package cn.vstore.appserver.api.payment.fortumo;
...
import cn.vstore.appserver.api.payment.shenzhoufu.SZFOrderPayUtil;
import cn.vstore.appserver.api.payment.shenzhoufu.ServerConnSzxUtils;
...
@Controller
public class SelectFortumoPayApi {
	protected static final Logger logger = LoggerFactory.getLogger(SelectFortumoPayApi.class);
	...
	@RequestMapping(value = "/api/ftmpay/order/{propsId:.+}", method = {
	...
	public String excuteApi(
		...
		String loginId = request.getParameter("loginId");//会员登陆帐号
...
//		String denomination = request.getParameter("cardMoney");//卡面额
		...
		String propsType = request.getParameter("propsType");//道具类型
		String nums = request.getParameter("nums");//道具数量
...
//		String memId = "2";//request.getParameter("memId");//购买用户ID系统唯一标识(网讯会员)
		orderPayForm.setMerusername("FORTUMO");
		...
		orderPayForm.setPaytype(PayType.FORTUMO);//表示fortumo
