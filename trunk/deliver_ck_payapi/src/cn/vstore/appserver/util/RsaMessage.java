...
public class RsaMessage {
	 ...
	 * @throws IOException 
	  */
	 public static String getStrDSAInfo(String signStr) throws IOException{
		URL url = Thread.currentThread().getContextClassLoader().getResource("100016_SignKey.pub");
		String tempUrl = url.toString();
	    String publicKeyFilePath = tempUrl.substring(tempUrl.indexOf("/"),tempUrl.length());
	    System.out.println("publickey======"+publicKeyFilePath);
//		String publicKeyFilePath = "F:\\100016_SignKey.pub";
		...
		try {
			encryptMessage = rsa.encrypt(signStr, publickKey);
//			System.out.println("公钥加密后："+encryptMessage);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @throws IOException 
	  */
	 public static String getStrDSADecInfo(String signStr) throws IOException{
		URL url = Thread.currentThread().getContextClassLoader().getResource("100016_SignKey.pub");
		String tempUrl = url.toString();
	    String publicKeyFilePath = tempUrl.substring(tempUrl.indexOf("/"),tempUrl.length());
	    System.out.println("publickey======"+publicKeyFilePath);
		...
		String encryptMessage = null;
		try {
			encryptMessage = rsa.decrypt(signStr, publickKey);
//			System.out.println("公钥加密后："+encryptMessage);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		 String publicKeyFilePath = "E:/workspace/ckapi/webapp/WEB-INF/classes/100016_SignKey.pub";
//		 String publicKeyFilePath = "F:\\rsa_pub_key.pem";
//		 String privateKeyFilePath = "F:\\100005.pri";
		 URL url = Thread.currentThread().getContextClassLoader().getResource("100016_SignKey.pub");
		 String tempUrl = url.toString();
		 System.out.println(tempUrl);
	     String publicKeyFilePath = tempUrl.substring(tempUrl.indexOf("/"),tempUrl.length());
	     System.out.println("fullFileName==="+publicKeyFilePath);
...
//		 RSAPrivateKey privateKey = RsaMessage.initPrivateKey(privateKeyFilePath);
		 ...
		 String decryptMessage = rsa.decrypt("s8us/fe6DAT8bXu5xVd5CkOe51Sp1u91jWPfkbl/ZulCxW3ytZxbX8EaEX7WP6UV5/Ct2I5TkUm07VwfzK1DWYXG0YLXxN1z7n796l8LKzmo2SCJw3XrAlLN22yoduJcx7awyYFoKTRcQ8DAKBWOWWxntRiTBdcbxn58KbR6vcA=", publickKey);
...
//		System.out.println("测试加密===="+getStrDSAInfo("sdfsd"));
	}
}